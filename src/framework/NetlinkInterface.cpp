...
namespace analyzer::framework::net
{
    ...
    static InterfaceInformation* GetCorrectInterface (const uint32_t index, const uint8_t family, const std::list<InterfaceInformation>& interfaces) noexcept
    {
        for (const auto& interface : interfaces)
        {
            ...
            {
                if (family == AF_UNSPEC) {
                    return const_cast<InterfaceInformation*>(&interface);
                }
                else if (interface.interfaceFamily == AF_UNSPEC || interface.interfaceFamily == family) {
                    return const_cast<InterfaceInformation*>(&interface);
                }
            }
        }
    ...
    bool NetlinkRequester::NetlinkInterfaceParser (void* const data, uint32_t length, std::list<InterfaceInformation>& interfaces, const uint16_t types, const bool onlyRunning) const noexcept
    {
        ...
        for ( ; NLMSG_OK(msg, length) == true; msg = NLMSG_NEXT(msg, length))
        {
                ...
                for ( ; RTA_OK(attribute, tailLength) == true; attribute = RTA_NEXT(attribute, tailLength))
                {
                    ...
                    switch (attribute->rta_type)
                    {
                        ...
                        case IFLA_BROADCAST:  // Interface broadcast MAC address.
                            ...
                            break;
                        default:
                            LOG_TRACE("NetlinkRequester.NetlinkInterfaceParser: Available additional interface information of type: ", attribute->rta_type, '.');
                            break;
                    }
                }
            ...
            else if (msg->nlmsg_type == NLMSG_ERROR)
            {
                ...
                LOG_ERROR("NetlinkRequester.NetlinkInterfaceParser: Error Netlink message with code: ", error->error, '.');
                return false;
            }
            else
            {
                LOG_WARNING("NetlinkRequester.NetlinkInterfaceParser: Unhandled Netlink message of type: ", msg->nlmsg_type, '.');
                return false;
            }
        }
        LOG_ERROR("NetlinkRequester.NetlinkInterfaceParser: Incorrect input message of type: ", msg->nlmsg_type, '.');
        return false;
    }
    ...
    bool NetlinkRequester::NetlinkAddressParser (void* const data, uint32_t length, std::list<InterfaceInformation>& addresses, const bool notEnrich) const noexcept
    {
        ...
        for ( ; NLMSG_OK(msg, length) == true; msg = NLMSG_NEXT(msg, length))
        {
            ...
            if (msg->nlmsg_type == RTM_NEWADDR)  // Get information about system interfaces addresses.
            {
                ...
                if (addr->ifa_family == AF_INET && interfaceFamily == AF_INET6)
                {
                    LOG_INFO("NetlinkRequester.NetlinkAddressParser: Skip IPv4 address.");
                    continue;
                }
                else if (addr->ifa_family == AF_INET6 && interfaceFamily == AF_INET)
                {
                    LOG_INFO("NetlinkRequester.NetlinkAddressParser: Skip IPv6 address.");
                    continue;
                }
                ...
                for ( ; RTA_OK(attribute, tailLength) == true; attribute = RTA_NEXT(attribute, tailLength))
                {
                    ...
                    switch (attribute->rta_type)
                    {
                        ...
                        case IFA_MULTICAST:  // Interface multicast address.
                            ...
                            break;
                        default:
                            LOG_TRACE("NetlinkRequester.NetlinkAddressParser: Available additional addresses information of type ", attribute->rta_type, '.');
                            break;
                    }
                }
            ...
            {
                const struct nlmsgerr* const error = static_cast<struct nlmsgerr*>(NLMSG_DATA(msg));
                LOG_ERROR("NetlinkRequester.NetlinkAddressParser: Error Netlink message with code: ", error->error, '.');
                return false;
            }
            else
            {
                LOG_WARNING("NetlinkRequester.NetlinkAddressParser: Unhandled Netlink message of type: ", msg->nlmsg_type, '.');
                return false;
            }
        }
        LOG_ERROR("NetlinkRequester.NetlinkAddressParser: Incorrect input message of type: ", msg->nlmsg_type, '.');
        return false;
    }
    ...
    bool NetlinkRequester::NetlinkRouteParser (void* const data, uint32_t length, std::list<RouteInformation>& routes, const uint8_t types) const noexcept
    {
        ...
        for ( ; NLMSG_OK(msg, length) == true; msg = NLMSG_NEXT(msg, length))
        {
            ...
            if (msg->nlmsg_type == RTM_NEWROUTE)  // Get information about system interfaces routes.
            {
                ...
                if (route->rtm_family == AF_INET && interfaceFamily == AF_INET6)
                {
                    LOG_INFO("NetlinkRequester.NetlinkRouteParser: Skip IPv4 route.");
                    continue;
                }
                else if (route->rtm_family == AF_INET6 && interfaceFamily == AF_INET)
                {
                    LOG_INFO("NetlinkRequester.NetlinkRouteParser: Skip IPv6 route.");
                    continue;
                }
                ...
                for ( ; RTA_OK(attribute, tailLength) == true; attribute = RTA_NEXT(attribute, tailLength))
                {
                    ...
                    switch (attribute->rta_type)
                    {
                        ...
                        case RTA_PRIORITY:  // Route priority.
                            ...
                            break;
                        default:
                            LOG_TRACE("NetlinkRequester.NetlinkRouteParser: Available additional route information of type ", attribute->rta_type, '.');
                            break;
                    }
                }
            ...
            {
                const struct nlmsgerr* const error = static_cast<struct nlmsgerr*>(NLMSG_DATA(msg));
                LOG_ERROR("NetlinkRequester.NetlinkRouteParser: Error Netlink message with code: ", error->error, '.');
                return false;
            }
            else
            {
                LOG_WARNING("NetlinkRequester.NetlinkRouteParser: Unhandled Netlink message of type: ", msg->nlmsg_type, '.');
                return false;
            }
        }
        LOG_ERROR("NetlinkRequester.NetlinkRouteParser: Incorrect input message of type: ", msg->nlmsg_type, '.');
        return false;
    }
        ...
        : interfaceFamily(family)
    {
        ...
        {
            sock.reset(nullptr);
            LOG_ERROR("NetlinkRequester.NetlinkRequester: Error occurred while initialize Socket class.");
        }
    }
